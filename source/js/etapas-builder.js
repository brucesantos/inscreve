...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (btnSalvarEtapa) {
    btnSalvarEtapa.addEventListener('click', function() {
      ...
      // Formatar as datas para exibição (DD/MM/AAAA)
      const dataInicioFormatada = dataInicio ? formatarData(dataInicio) : 'DD/MM/AAAA';
      const dataFimFormatada = dataFim ? formatarData(dataFim) : 'DD/MM/AAAA';
  ...
  if (colunaDeEtapas && typeof Sortable !== 'undefined') {
    sortableInstance = Sortable.create(colunaDeEtapas, {
      animation: 150,
      handle: '.drag-handle', // Apenas o elemento com a classe drag-handle pode iniciar o arrasto
      ghostClass: 'card-de-etapa-ghost', // Classe aplicada ao elemento fantasma durante o arrasto
      chosenClass: 'card-de-etapa-chosen', // Classe aplicada ao elemento escolhido
      dragClass: 'card-de-etapa-drag', // Classe aplicada durante o arrasto
      forceFallback: false, // Usar o fallback nativo para melhor desempenho
      fallbackTolerance: 3, // Tolerância em pixels antes de iniciar o arrasto
      scrollSpeed: 40, // Velocidade de rolagem quando próximo às bordas
      scrollSensitivity: 80, // Distância das bordas para iniciar a rolagem
      ...
      onEnd: function(evt) {
        ...
        // Aqui você pode adicionar código para salvar a nova ordem
        console.log('Nova ordem estabelecida');
        
        // Adicionar efeito de destaque temporário ao elemento movido
        const item = evt.item;
  ...
  componentesList.forEach(componente => {
    componente.addEventListener('click', function(e) {
      ...
      const nomeComponente = this.querySelector('.fw-bold').textContent;
      
      // Adicionar nova etapa
      adicionarNovaEtapa(tipoComponente, nomeComponente);
      
      // Fechar o modal
      const modal = bootstrap.Modal.getInstance(document.getElementById('modalComponentes'));
      modal.hide();
    });
  });
  
  // Função para adicionar nova etapa
  function adicionarNovaEtapa(tipo, nome, parentElement = null, dataInicio = 'DD/MM/AAAA', dataFim = 'DD/MM/AAAA') {
    // Template para nova etapa
    const novaEtapaHTML = `
      <div class="card card-de-etapa mb-3">
        <div class="card-body">
              ...
              <div class="acoes d-flex gap-2">
                <button type="button" class="criar-filho btn btn-outline-primary p-0" style="width: 38px; height: 38px;">
                  <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M9.875 6.125H6.125V9.875H9.875V6.125ZM6.125 4.25H9.875C10.8906 4.25 11.75 5.10938 11.75 6.125V9.875C11.75 10.9297 10.8906 11.75 9.875 11.75H8.9375V15.5C8.9375 16.3984 9.60156 17.0625 10.5 17.0625H14.25V16.125C14.25 15.1094 15.0703 14.25 16.125 14.25H19.875C20.8906 14.25 21.75 15.1094 21.75 16.125V19.875C21.75 20.9297 20.8906 21.75 19.875 21.75H16.125C15.0703 21.75 14.25 20.9297 14.25 19.875V18.9375H10.5C8.58594 18.9375 7.0625 17.4141 7.0625 15.5V11.75H6.125C5.07031 11.75 4.25 10.9297 4.25 9.875V6.125C4.25 5.10938 5.07031 4.25 6.125 4.25ZM19.875 16.125H16.125V19.875H19.875V16.125Z" fill="currentColor"/>
                  </svg>
                </button>
    ...
    if (parentElement) {
      ...
      if (sortableInstance) {
        sortableInstance.option("animation", 150); // Isso força o Sortable a atualizar seus elementos
      }
    }
  }
  ...
  function configurarEventosCard(card) {
    ...
    if (btnCriarFilho) {
      btnCriarFilho.addEventListener('click', function() {
        // Abrir o modal de componentes para escolher o tipo de filho
        const modal = new bootstrap.Modal(document.getElementById('modalComponentes'));
        
        // Armazenar referência ao card pai para uso posterior
        window.cardPaiAtual = this.closest('.card-de-etapa').querySelector('.children');
        
        // Modificar o comportamento do modal para criar filhos
        const componentesList = document.querySelectorAll('.list-group-item[data-tipo-componente]');
        
        // Remover listeners antigos e adicionar novos
        componentesList.forEach(componente => {
          // Clonar o elemento para remover todos os event listeners
          const novoComponente = componente.cloneNode(true);
          componente.parentNode.replaceChild(novoComponente, componente);
          
          // Adicionar novo event listener
          novoComponente.addEventListener('click', function(e) {
            e.preventDefault();
            ...
            // Adicionar nova etapa como filho
            if (window.cardPaiAtual) {
              adicionarNovaEtapa(tipoComponente, nomeComponente, window.cardPaiAtual);
              window.cardPaiAtual = null; // Limpar a referência
            }
  ...
  function inicializarSortableFilhos(childrenContainer) {
    if (typeof Sortable !== 'undefined') {
      Sortable.create(childrenContainer, {
        ...
        group: 'nested', // Permite arrastar entre diferentes listas
        onStart: function(evt) {
          document.body.classList.add('dragging-card');
        },
        onEnd: function(evt) {
